...
Promise.all([
	...
	// Alcaldías
	for(i=0; i<alcaldias.records.length; i++){
	...
	var marcadoresTodos = [];
	// Escuelas privadas
	for(i=0; i<privadas.records.length; i++){
		// Creamos el marcador y le ponemos una leyenda con el nombre de la escuela y la ubicación
		marcador=L.marker([privadas.records[i][10], privadas.records[i][11]], {icon: iconPrivada})
	...
	var privadas = L.layerGroup(marcadores);
	...
	var marcadores = [];
	for(i=0; i<publicas.records.length; i++){
		// Creamos el marcador de escuela pública y le agregamos una leyenda
		marcador=L.marker([publicas.records[i][5], publicas.records[i][6]], {icon: iconPublica})
		...
		del = publicas.records[i][3].match(re);
	...
	var publicas = L.layerGroup(marcadoresPublicas);

	// Agregar marcadores al mapa
	marcadoresTotal.addTo(mapaAlcaldias);
	// Agregas las layers al mapa
	mapaAlcaldias.layers = [privadas, publicas];
